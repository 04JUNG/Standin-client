...
> **현행 편차(docs/05 §8):** Toast 컴포넌트는 아직 없다. 단축키 변경 결과는 설정 화면 안의
> `aria-live` 문구로 알리고, 등록 실패는 복구 행동(다른 키 선택)이 필요하므로 아래 규칙에 따라
> 인라인 오류 + 되돌리기 버튼으로 처리한다. Toast 도입 시 성공 알림만 옮긴다.
