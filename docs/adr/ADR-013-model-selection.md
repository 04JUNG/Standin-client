...
- `ModelCharacter.source`(`builtin | store | user`)와 섹션 구조가 에셋 스토어·모델 불러오기·
  ...
  추가로 끝난다.
- **개정(2026-09-07): 체형은 인물별로 고른다.** 처음에는 저장 배치 하나에 체형 하나로 두고
  "다인 컷이 반은 남성, 반은 여성이 되면 안 된다"고 봤는데, 그건 *실수로* 섞이는 경우의
  이야기였다. 한 컷에 남녀가 같이 있는 장면은 흔하고, 그때 배치 하나에 체형 하나면 같은 컷을
  두 번 저장해야 한다. 고정값은 `characterId`(작업 기본)와 `characterByPerson`(인물별
  덮어쓰기) 둘로 나뉘고, 후보 화면의 인물 카드에서 고른다. 저장 화면은 인물마다 다르면
  이름 하나를 대지 않고 "인물마다 고른 모델로 저장했습니다"라고 말한다.
- **분석 전에는 고르게 하지 않는다.** 홈·입력 미리보기·바 동작 화면의 선택 UI는 걷어냈다.
  그 시점에는 인물이 몇 명인지도 모르고, 러프를 넣는 흐름에 선택지를 하나 더 얹으면
  캡처·업로드가 묻힌다(CLAUDE.md §11). 기본 체형은 모델 화면에서만 정한다.
