...
- 전역 단축키(`Ctrl+Alt+S`), 앱 상단의 **플로팅 바** 버튼, 또는 **앱 창 최소화**로 진입합니다. 최소화하면 작업 표시줄로 숨는 대신 화면 위 동그란 버튼으로 남습니다.
...
- **전역 캡처 바 열기**: `Ctrl+Alt+S`(macOS `⌘⌥S`). 앱이 백그라운드여도 동작하며 설정에서 변경할 수 있다. 이 키는 **캡처를 바로 시작하지 않고 플로팅 바를 연다**. 캡처는 바에서 `C` 또는 캡처 버튼으로 시작한다(ADR-008).
- **앱 내**: 화면별 고정. 예) 저장 `Ctrl+S`, 분석 시작 `Ctrl+Enter`, 취소 `Esc`.

기본값이 `Ctrl+Shift+S`가 아닌 이유: 클립스튜디오 기본 "다른 이름으로 저장"과 충돌해 CSP가 먼저 떠 있으면 등록에 실패한다.

전역 단축키는 데스크톱(`npm run tauri dev`)에서만 등록된다. 브라우저 개발 모드에서는 같은 키가 앱이 활성일 때만 동작하는 대체 경로로 내려가고, 설정 화면이 그 사실을 표시한다. 데스크톱에서 **등록에 실패하면** 앱 셸 상단 배너와 플로팅 바 경고 아이콘으로 알린다.
