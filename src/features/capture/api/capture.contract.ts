 ...
 *
 * ⚠ 단위는 OS마다 다르다 — Windows는 물리 픽셀, macOS는 points다. 프론트는 이 값을
 * 해석하지 않고 오버레이를 띄울 때 네이티브로 그대로 돌려보내기만 한다. 계산에 쓰려면
 * 단위를 먼저 확인해야 한다.
 *
 * 캡처 대상과 오버레이 위치가 각자 정해지면 다른 화면 사진을 엉뚱한 화면에 띄우게
 * 된다(듀얼 모니터에서 실측). 그래서 캡처한 쪽이 고른 모니터를 그대로 넘긴다.
 */
